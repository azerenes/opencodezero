...
const USER_AGENT = `opencode/${InstallationChannel}/${InstallationVersion}/${Flag.PIXI_CLIENT}`
...
export const layer: Layer.Layer<Service, never, Requirements> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const source = Flag.PIXI_MODELS_URL || "https://models.dev"
    ...
    const loadFromDisk = fs.readJson(Flag.PIXI_MODELS_PATH ?? filepath).pipe(
    ...
    const populate = Effect.gen(function* () {
      ...
      if (Flag.PIXI_DISABLE_MODELS_FETCH) return {}
    ...
    if (!Flag.PIXI_DISABLE_MODELS_FETCH && !process.argv.includes("--get-yargs-completions")) {
