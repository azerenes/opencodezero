...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .pixi/ subdirectory too)
  const candidates = [path.join(baseDir, "pixi.json"), path.join(baseDir, "pixi.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".pixi", "pixi.json"), path.join(baseDir, ".pixi", "pixi.jsonc"))
  }
